...
6. `analysis_complete` is analyzer-local. `review_complete=false` means do not conclude: follow every cursor present. After all four cursor names are absent and graph remediation is exhausted, terminal coverage is complete only when `review_complete_when_pages_exhausted=true`; otherwise report incomplete coverage. Any explicit artifact omission—including binary, oversized, unsafe, non-regular, type-changed, or unmerged—or cursor/remediation failure means incomplete coverage. Never read an unsafe path or binary bytes as source or fallback. Complete artifact coverage does not add indexed source languages; Graphr indexes only Rust and Python.
