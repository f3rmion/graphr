...
3. Call Graphr `changes` once without a cursor with `depth: 6` and `max_nodes: 50`. Exhaust every `files_next_cursor`, `diff_next_cursor`, `artifacts_next_cursor`, and `graph_next_cursor` returned by any page by calling `changes` with the same base, depth, max-nodes, and dependency-mode arguments plus the exact cursor. These pages share one immutable snapshot; `max_nodes` changes graph page size, never snapshot coverage. Do not make another cursorless call. If a cursor is stale or fails, report incomplete coverage.
...
6. After exhausting every cursor and graph remediation, coverage is complete only when `review_complete_when_pages_exhausted=true`. Any explicit artifact omission—including binary, oversized, unsafe, non-regular, type-changed, or unmerged—or any false coverage flag means incomplete coverage. Never read an unsafe path or binary bytes as source or fallback. Complete artifact coverage does not add indexed source languages; Graphr indexes only Rust and Python.
